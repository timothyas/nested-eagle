...
def draw_box(ax, hds, **kwargs):
    ...
    n_x = hds.attrs["field_shape"][1]
    n_y = hds.attrs["field_shape"][0]
...
def nested_scatter(ax, xds, hds, varname, **kwargs):
    n_conus = len(hds["latitudes"])
    mappables = []
    for slc, s in zip(
